...
{
	public class GelBubble1 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			Projectile.rotation += 0.08f + Projectile.velocity.Length() * 0.02f;
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			float wobble = (float)Math.Sin(Main.GlobalTimeWrappedHourly * 8.2f + Projectile.whoAmI * 0.45f);
			...
			Vector2 visualScale = Projectile.scale * new Vector2(
				1f + 0.06f * wobble + speedWobble + stretchFactor,
				1f - 0.05f * wobble - speedWobble * 0.45f - stretchFactor * 0.35f
			);
			Color drawColor = Projectile.GetAlpha(lightColor);

			// Motion-smear behind the bubble to make the sphere feel stretched by drag.
			for (int i = 3; i >= 1; i--)
			{
				float trailT = i / 3f;
