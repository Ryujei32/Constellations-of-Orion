...
{
	// ⭐ MASTER MODE RELIC (100% in Master Mode only)
	npcLoot.Add(ItemDropRule.MasterModeCommonDrop(
		ModContent.ItemType<Content.Items.Placeables.Relics.CosmicAstraeusRelic>()
	));

	// ⭐ MASTER MODE PET (RARE)
	npcLoot.Add(ItemDropRule.MasterModeDropOnAllPlayers(
		ModContent.ItemType<Content.Items.Pets.StarrySundae>(),
		4 // 1 in 4 chance (adjust if you want rarer)
	));

	// ⭐ RARE WEAPON DROP (Gelatinous Spray)
	npcLoot.Add(ItemDropRule.Common(
		ModContent.ItemType<Content.Items.Weapons.GelatinousSpray>(),
		5 // 1 in 5 chance
	));
    
    npcLoot.Add(ItemDropRule.Common(
		ModContent.ItemType<Content.Items.Weapons.StellarLash>(),
		5 // 1 in 5 chance
	));

	// ⭐ CONSTELLITE BAR DROP (RANDOM STACK)
	npcLoot.Add(ItemDropRule.Common(
		ModContent.ItemType<Content.Items.Materials.ConstelliteBar>(),
		1,
		18,
		32
	));

	// ⭐ GEL DROP (RANDOM STACK)
	npcLoot.Add(ItemDropRule.Common(
		ItemID.Gel,
		1,
		40,
